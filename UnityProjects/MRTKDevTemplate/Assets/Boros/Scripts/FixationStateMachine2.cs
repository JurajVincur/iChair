...
public class FixationStateMachine2 : MonoBehaviour
{
    [Header("References")]
    ...
    public float state2NearFirstFixationDistance = 0.05f;
    ...
    private Vector3 state2AngleStartFixation;
    private bool hasState2AngleStartFixation = false;
    ...
    {
        if (!hasState2AngleStartFixation)
        {
            float distanceToFirstFixation = Vector3.Distance(fixationPoint, firstFixationPoint);

            Debug.Log("Distance to original first fixation: " + distanceToFirstFixation.ToString("F3"));

            if (distanceToFirstFixation <= state2NearFirstFixationDistance)
            {
                state2AngleStartFixation = fixationPoint;
                hasState2AngleStartFixation = true;

                Debug.Log("Stored state 2 angle start fixation near original first fixation: " + state2AngleStartFixation);
            }
            else
            {
                Debug.Log("Fixation is not near original first fixation. Waiting for angle start fixation.");
            }
        ...
        if (!hasSecondFixation)
        {
            ...
            Debug.Log("State 2 valid-distance candidate fixation: " + secondFixationPoint);
    ...
    private void EvaluateState2Selection()
    {
        ...
        if (!hasState2AngleStartFixation)
        {
            Debug.LogWarning("State 2 angle start fixation is missing.");
            hasSecondFixation = false;
            secondFixationPoint = Vector3.zero;
            return;
        }
        ...
        // Distance validation still uses the ORIGINAL first fixation
        Vector3 validationVectorWorld = secondFixationPoint - firstFixationPoint;
        float state2VectorDistance = validationVectorWorld.magnitude;
        ...
        Debug.Log("State 2 validation distance from ORIGINAL first fixation: " + state2VectorDistance.ToString("F3"));
        ...
        // Angle calculation uses the NEW near fixation, not the original first fixation
        Vector3 state2VectorWorld = secondFixationPoint - state2AngleStartFixation;

        if (state2VectorWorld.magnitude < 0.001f)
        {
            Debug.Log("State 2 angle vector is too small. Stay in state 2.");
            hasSecondFixation = false;
            secondFixationPoint = Vector3.zero;
            return;
        }
    ...
    private void ResetFixations()
    {
        ...
        hasSecondFixation = false;
        hasState2AngleStartFixation = false;

        firstFixationPoint = Vector3.zero;
        secondFixationPoint = Vector3.zero;
        state2AngleStartFixation = Vector3.zero;
    }
}
